...
import { GetAddressesUserModel  } from '../../../types';
import { GetAddressesUserComposer } from '../../../main/composer/user';
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    if (body) {
      const { action }: GetAddressesUserModel = parseBody(body);
      const data = await GetAddressesUserComposer.compose(headers.authorization!)
