...
import { getDatabaseService } from '../../cdk/auroradb';
import { getFunctions } from '../../cdk/lambda';

describe('AuroraDB', () => {
  test('returns a serverless aurora', () => {
    const app = new App();
    const stack = new Stack(app, 'ApiTestStack', { env: { account: '123456789', region: 'us-east-1' } });
    const vpc = getVpc(stack)
    const host = getBastionHost(stack, vpc)
    getDatabaseService(stack, vpc, host, getFunctions(stack));
